...
const evaluationLoader = () => new NodeEvaluationDataLoader(
...
try {
  ...
  const ai = new AIAnalysisService(repository, new ManualJsonAIProvider(), evaluationLoader())
  ...
  const slowAi = new AIAnalysisService(concurrentRepo, new ManualJsonAIProvider(), evaluationLoader())
  ...
  const failingAi = new AIAnalysisService(failingRepo, new ManualJsonAIProvider(), evaluationLoader())
  ...
  const lineBreak = quoteRepair.repair('清爽 自然', '用户评价：清爽\r\n\t自然，适合冰饮。')
  assert.equal(lineBreak.quoteAutoRepaired, true)
  assert.equal(lineBreak.repairedQuote, '清爽\r\n\t自然')
  const nfkc = quoteRepair.repair('AI饮料', '品牌发布ＡＩ饮料新品。')
  assert.equal(nfkc.quoteAutoRepaired, true)
  assert.equal(nfkc.repairedQuote, 'ＡＩ饮料')
  const punctuationChanged = quoteRepair.repair('清爽,自然', '用户评价：清爽，自然。')
  assert.equal(punctuationChanged.repairMethod, 'not_found', '中英文标点不属于安全等价修复')
  const spliced = quoteRepair.repair('Keurig Dr Pepper is entering 2026', 'Keurig Dr Pepper (NASDAQ: KDP) is entering 2026')
  assert.equal(spliced.repairMethod, 'not_found', '删除原文中间片段的拼接引文不得自动修复')
  ...
  const validationAi = new AIAnalysisService(validationRepo, new ManualJsonAIProvider(), evaluationLoader())
